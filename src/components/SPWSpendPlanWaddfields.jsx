...
const getPlans = async (props) => {
    try{
        const res = await fetch(`${process.env.NEXT_PUBLIC_NEXTAUTH_URL}/api/spending-plan-alt`,{
           cache: 'no-store',
           method: "GET",
           headers: headers(),
        });
...
const getCategories = async () => {
    try{
        const res = await fetch(`${process.env.NEXT_PUBLIC_NEXTAUTH_URL}/api/category`,{
           cache: 'no-store',
           method: "GET",
           headers: headers(),
        });
    ...
    return(
       ...
       <div className="flex flex-row  w-full min-h-[50%] bg-white">
        ...
        {spendingplan.mycategories.map((lookup,index) =>
            ...
            <div className="border border-amber-500 w-[200px] p-2 ">Category:{lookup.$mycategoryId}{lookup.categorynotes}</div>
            <div className="border border-amber-500 w-[200px] p-2 ">{lookup.$title}</div>
            <div className="border border-amber-500 w-[200px] p-2 ">{lookup.planamount.$numberDecimal}</div>
            </div>
            )}
        </> 
        ): `You don't have a plan set up for ${props.fmonth}/${props.fyear}`}
        </div>
